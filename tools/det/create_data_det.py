...
from coperception_origin.utils import mapping
from coperception_origin.configs import Config
from coperception_origin.utils.data_util import voxelize_occupy
from coperception_origin.utils.obj_util import *
from coperception_origin.utils.nuscenes_pc_util import (
...
from coperception_origin.utils.v2x_sim_scene_split_v1.parser import parse_scene_files
...
def create_data(coperception_dataset, current_agent, scene_begin, scene_end, scene_splits, args_savepath):
    ...
    for scene_idx in res_scenes[scene_begin:scene_end]:
        ...
        while channel_flag:
            ...
            pc = all_pc.points  #(4, 43275)
            _, sort_idx = np.unique(all_times, return_index=True)

            # Preserve the item order in unique_times
            unique_times = all_times[np.sort(sort_idx)] #(1,)
            num_sweeps = len(unique_times)  # 1
            ...
            for tid in range(num_sweeps):
                ...
                points_idx = np.where(all_times == _time)[0]    # points_idx=(43275,)
                _pc = pc[:, points_idx]
            ...
            corresponding_sample_token = curr_sample_data["sample_token"]   # 帧token
            ...
            for ann_token in corresponding_sample_rec["anns"]:
                ...
                '''box_data_dict["category_instance_token"]=1'''
                flag = False
                ...
                (
                    ...
                    curr_sample_data,
                    instance_token,
                    nsweeps_back=config.nsweeps_back,       #1
                    nsweeps_forward=config.nsweeps_forward, #0
                )
            ...
            adj_seq_cnt += 1
            if adj_seq_cnt == config.num_adj_seqs:  # 1
                ...
                curr_save_box_dict = save_box_dict_list[0]  # curr_save_box_dict=box_data_dict["category/instance_boxes_哪个instance(token)]
                gt_box_dict = dict()
                ...
                flag = False    #true-No more keyframes
                for _ in range(config.num_keyframe_skipped + 1):
                    if curr_sample["next"] != "":                   #if next，next=curr_sample
...
def convert_to_dense_bev(seq_data_dict, config):
    ...
    # Load point cloud
    pc_list = []
    for i in range(num_sweeps): # i=0
        pc = data_dict["pc_" + str(i)]  #(4,43275)
        pc_list.append(pc.T)            #[(43275,4)]
    ...
    origins[:, 3] = 1.0 #(0,0,0,1)
    pc_range = [        #[-32.0, -32.0, -3.0, 32.0, 32.0, 2.0]
    ...
    if config.is_cross_road:
        ...
        extents = config.area_extents
        for idx in range(origins.shape[0]):#origins(1,4) idx=0
            pts = pc_list[idx]
            ...
            visibility_maps.append( #arry (3407872,)
...
def convert_to_sparse_bev(config, dense_bev_data, use_motion_state=False):
    ...
    vis_occupy_indices = np.asarray(np.where(visibility_maps > 0))
    vis_free_indices = np.asarray(np.where(visibility_maps < 0))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r",
        "--root",
        default="/home/suwanfang/coperception_old/data/V2X-Sim-1-origin",
        type=str,
        help="Root path to coperception_origin dataset",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-p",
        "--savepath",
        default="/home/suwanfang/coperception_old/data/V2X-Sim-1.0-trainval/",
        type=str,
    ...
    parser.add_argument(
        "--to_agent", default=5, type=int, help="until which agent (index + 1)"
    )
    parser.add_argument(
        "--dataset_version", default='v1.0-mini', type=str, help="[v2.0 / v2.0-mini]"
    )
    ...
    scene_files_loc = '/home/suwanfang/coperception_origin/coperception_origin/utils/v2x_sim_scene_split_v1'
    scene_splits = parse_scene_files(scene_files_loc)

    print(f'Parsed files will be saved to {args.savepath}')

    for current_agent in range(3, args.to_agent):
