from coperception_origin.models.det.backbone.Backbone import *
import numpy as np
...
class DetModelBase(nn.Module):
    ...
    def build_neighbors_feature_list(
        ...
        for j in range(num_agent):
            if j != agent_idx:
                ...
                warp_feat = DetModelBase.feature_transformation(self,
                    b,
                    j,
                    agent_idx,
                    local_com_mat,
    ...
    def get_cls_loc_result(self, x):
        ...
        cls_preds = cls_preds.permute(0, 2, 3, 1).contiguous() # [5个agent,12个anchor,256,256]
        # [5,x=256x256个体素x每个体素有6个anchor=393216个anchor,类数=2(是/否汽车) 每个anchor分成两类的概率]
        cls_preds = cls_preds.view(cls_preds.shape[0], -1, self.category_num)

        # Detection head
        # torch.Size([5, 36(anchor_num_per_loc * box_code_size), 256, 256])
        loc_preds = self.regression(x)
        # torch.Size([5, 256, 256, 36(anchor_num_per_loc * box_code_size)])
        loc_preds = loc_preds.permute(0, 2, 3, 1).contiguous()
        # torch.Size([5个agent, 256, 256, 每个体素6个anchor, 1, 每个anchor要6个编码])
        ...
        # [5, 256,256,6,1,每个anchor的编码6]; [5,256x256个体素x每个体素有6个anchor,每个anchor分类数=2(是/否汽车)]
...
class ClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        # binary：len(config.anchor_size)=6
...
class DetModelBase_encoder(nn.Module):
    """Abstract class. The super class for all detection models.
    ...
    def __init__(
        ...
        super(DetModelBase_encoder, self).__init__()
