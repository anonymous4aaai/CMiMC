...
from coperception_origin.models.det.base import IntermediateModelBase
from coperception_origin.models.det.base import IntermediateModelBase_encoder
...
class DiscoNet(IntermediateModelBase):
    ...
    def __init__(self, config, layer=3, in_channels=13, kd_flag=True, num_agent=5, compress_level=0, only_v2i=False,MMI_flag=False):
    ...
    def forward(self, bevs, trans_matrices, num_agent_tensor, batch_size=1):
        ...
        local_feat_L_update_list = []
        for i in range(self.agent_num):
            # [4, 6, 256, 32, 32]
            temp = super().build_local_communication_matrix(feat_list)
            local_feat_L_update_list.append(temp)
            ...
            for i in range(num_agent):
                ...
                if super().outage():
                    ...
                    for k in range(nb_len):
                        # local_feat_L_update_list[i]: [4, 6, 256, 32, 32]
                        if k==i: # feat_L_update[b,i] = tg_agent = self.neighbor_feat_list[0] 已完成
                            continue
                        if k<i:
                            local_feat_L_update_list[i][b,k] = self.neighbor_feat_list[k+1] # x_3 [256, 32, 32]
                        if k>i:
                            local_feat_L_update_list[i][b,k] = self.neighbor_feat_list[k]
                    ...
                    for k in range(nb_len):
                        ...
                        agent_weight = torch.squeeze(
                            self.PixelWeightedFusion(cat_feat)
                        )
                    ...
                    for k in range(nb_len):
                        agent_weight = torch.div(tmp_agent_weight_list[k], sum_weight)  # [32,32]
                        agent_weight.expand([256, -1, -1])
                        agent_weight_list.append(agent_weight)

                    agent_wise_weight_feat = 0
                    for k in range(nb_len):
                        # agent_weight_list[k]: [32, 32] [w_{i->i}, 0->i, 1->i, ...]
        ...
        aligned_feat_L_list = []
        for i in range(self.agent_num):
            feat_L_align_mat = super().agents_to_batch(local_feat_L_update_list[i])
            aligned_feat_L_list.append(feat_L_align_mat)
        ...
        if self.kd_flag == 1:
            return (result, *decoded_layers, feat_fuse_mat, aligned_feat_L_list)
        else:
            # return result
            ...
            if self.config.split != "train":
                ...
                return (result, *decoded_layers, feat_fuse_mat, aligned_feat_L_list) # MMI
