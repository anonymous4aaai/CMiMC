...
from coperception_origin.utils.postprocess import *
...
def apply_nms_det(
    ...
    for box_preds, cls_preds, anchors in zip(
        ...
        for i in range(boxes_for_nms.shape[1]):
            ...
            # [256x256x6,1, 4 ,2] [256x256x6, 4 ,2]
            box_corners[:, i] = box_pred_corners

        class_selected = []
        # print(box_preds.shape, cls_preds.shape, total_scores.shape)
        for i in range(total_scores.shape[1]):  # total_scores [393216,1]
            selected_idx = non_max_suppression(
...
def cal_local_mAP(config, data, det_results, annotations):
    ...
    for p in range(pred_len):
        ...
        for k in range(len(gt_max_iou_idx)):
            ...
            # print(decode_box)
            decode_corner = center_to_corner_box2d( # [4,2]
...
def late_fusion(ego_agent, num_agent, result, trans_matrices, box_color_map):
    ...
    if len(result[ego_agent]) > 0:
        ...
        #
        result[ego_agent][0][0][0]["score"] = np.take(
            result[ego_agent][0][0][0]["score"], pick, axis=0
        )
        result[ego_agent][0][0][0]["selected_idx"] = np.take(
            result[ego_agent][0][0][0]["selected_idx"], pick, axis=0
        )
...
def visualization(config, data, box_colors, box_color_map, apply_late_fusion, savename=None, savename_grid=None):
    ...
    anchors_map = data["anchors_map"]   # torch.Size([256, 256, 6, 6]) h,w,几个anchor,code_6
    ...
    for p in range(pred_len):   # p=0
        for k in range(len(pred_selected)):
            ...
            if config.motion_state:
                cls_pred_state = pred_selected[k]["motion"]
            for corner_id in range(cls_pred_corners.shape[0]):
                corner_box = cls_pred_corners[corner_id]    # 编码[4,2]
                corners = coor_to_vis(
                    corner_box, area_extents=area_extents, voxel_size=voxel_size
                )
                c_x, c_y = np.mean(corners, axis=0)
                corners = np.concatenate([corners, corners[[0]]])   # [4,2]->[5,2]
        ...
        for k in range(len(gt_max_iou_idx)):
            ...
            if config.code_type[0] == "f":
                if config.pred_type == "motion":
                    ...
                    # motion b
                    if p == 0:
                        decode_box = bev_box_decode_np(encode_box, anchor) # [x, y, w, h, sin, cos]
                        cur_det.append(decode_box)
                    else:
                        decode_box = cur_det[k].copy()
                        decode_box[:2] += encode_box[:2]
                    ...
                    decode_box = bev_box_decode_np(encode_box, anchor)
                # print(decode_box) decode_box(6,) -> decode_corner (4,2)
                decode_corner = center_to_corner_box2d(
                    np.asarray([decode_box[:2]]),
                    np.asarray([decode_box[2:4]]),
                    np.asarray([decode_box[4:]]),
    ...
    m = np.stack([maps, maps, maps], axis=-1) #(256,256,3)
    m[m == 0] = 0.99
    m1 = m[:, :, 0]
    m2 = m[:, :, 1]
    m3 = m[:, :, 2]
    ...
    plt.imshow(maps, zorder=0,extent=[0,maps.shape[0],maps.shape[1],0])
    plt.xticks([])
    plt.yticks([])
