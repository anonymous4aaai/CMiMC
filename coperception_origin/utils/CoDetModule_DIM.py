...
from coperception_origin.utils.detection_util import *
from coperception_origin.utils.min_norm_solvers import MinNormSolver
import numpy as np
import torch
...
class FaFModule(object):
    def __init__(self, model, teacher, config, optimizer, criterion, kd_flag, MMI_flag,MILoss=None,optimizer_miloss=None):
        self.MMI_flag = MMI_flag
        if self.MMI_flag and config.split == "train":
            self.MILoss = MILoss
            self.optimizer_miloss = optimizer_miloss
            self.scheduler_miloss = torch.optim.lr_scheduler.MultiStepLR(
                optimizer_miloss, milestones=[50,100,150], gamma=0.5
            )
        ...
        if self.MGDA:
            ...
            self.scheduler = torch.optim.lr_scheduler.MultiStepLR(
                optimizer, milestones=[50,60,70,80,110,130], gamma=0.5
            )
    ...
    def step(self, data, batch_size, num_agent,alpha):
        ...
        if self.config.flag.startswith("when2com") or self.config.flag.startswith("who2com"):
            ...
            if self.kd_flag or self.config.flag == "disco":
                if self.kd_flag:
                    result, x_8, x_7, x_6, x_5, fused_layer, aligned_feat_L_list = self.model(
                        bev_seq, trans_matrices, num_all_agents, batch_size=batch_size)
                else:
                    result, x_8, fused_layer, aligned_feat_L_list = self.model(
                        bev_seq, trans_matrices, num_all_agents, batch_size=batch_size)
            else:
                result = self.model(
                    bev_seq, trans_matrices, num_all_agents, batch_size=batch_size
        ...
        '''MMI '''
        num_list = [num_all_agents[i][0].item() for i in range(num_all_agents.size()[0])]
        ...
        mi_loss = 0
        if self.config.flag == "disco" and self.MMI_flag:
            for i in range(num_max):
                feat_H_agent_i = fused_layer[batch_size * i : batch_size * (i + 1)] #前4个是第一个agent的4帧

                if i<num_min:
                    cnt_H += 1
                    feat_H_local = feat_H_agent_i # [4, 256, 32, 32]

                    cnt_L = 0
                    for j in range(num_min):
                        cnt_L += 1
                        feat_L_agent_j = aligned_feat_L_list[i][batch_size * j : batch_size * (j + 1)] #38
                        feat_L_local = feat_L_agent_j # [4, 256, 32, 32]

                        feat_L_local_prime = torch.cat((feat_L_local[1:],feat_L_local[0].unsqueeze(0)),dim=0)

                        lmiloss, gmiloss, tmiloss = self.MILoss(
                            feat_L_local, feat_L_local_prime,feat_H_local) # [4, 256, 32, 32]
                        cnt_tmiloss += 1
                        mi_loss += tmiloss

                    # print("num_list=",num_list)
                    for k in range(cnt_L,num_max):
                        temp_feat_L_local_list = []
                        temp_feat_H_local_list = []
                        temp_sample_id = [i for i in range(num_all_agents.size()[0])] #哪几帧没有参与协作(0-3)

                        for b in range(len(num_list)):
                            if num_list[b]>cnt_L:
                                temp_sample_id.remove(b)
                                feat_L_agent_k = aligned_feat_L_list[i][batch_size * k : batch_size * (k + 1)]
                                feat_H_agent_i = fused_layer[batch_size * i : batch_size * (i + 1)]
                                # raise
                                temp_feat_L_local_list.append(feat_L_agent_k[b:b+1]) # [1, 256, 32, 32]
                                temp_feat_H_local_list.append(feat_H_agent_i[b:b+1])

                        cnt_L += 1
                        num_coperate = len(temp_feat_L_local_list)
                        if num_coperate==0:
                            continue
                        feat_L_local = torch.cat(tuple(temp_feat_L_local_list), 0) # [b, 256, 32, 32]
                        feat_H_local = torch.cat(tuple(temp_feat_H_local_list), 0)

                        # shuffling
                        if num_coperate>1:
                            feat_L_local_prime = torch.cat((feat_L_local[1:],feat_L_local[0].unsqueeze(0)),dim=0)
                        else:
                            choice1 = random.choice(temp_sample_id)
                            feat_L_local_prime = aligned_feat_L_list[i][batch_size * 0 : batch_size * 1][choice1:choice1+1]

                        lmiloss, gmiloss, tmiloss = self.MILoss(
                            feat_L_local, feat_L_local_prime, feat_H_local)
                        # print("agent",i,k,lmiloss.item(),gmiloss.item(),tmiloss.item())
                        cnt_tmiloss += 1
                        mi_loss += tmiloss
                    ...
                    temp_sample_id = [i for i in range(num_all_agents.size()[0])]
                    temp_sample_id_cope = []
                    for b in range(len(num_list)):
                        if num_list[b]>cnt_H:
                            temp_sample_id_cope.append(b)
                            temp_sample_id.remove(b)
                    # print(temp_sample_id_cope)

                    cnt_H += 1

                    for j in range(num_max):
                        feat_L_agent_j = aligned_feat_L_list[i][batch_size * j : batch_size * (j + 1)]
                        temp_feat_L_local_list = []
                        temp_feat_H_local_list = []
                        for b in temp_sample_id_cope:
                            if cnt_L<num_list[b]:
                                temp_feat_L_local_list.append(feat_L_agent_j[b:b+1])
                                temp_feat_H_local_list.append(feat_H_agent_i[b:b+1])
                        cnt_L+=1
                        num_coperate_L = len(temp_feat_L_local_list)

                        feat_L_local = torch.cat(tuple(temp_feat_L_local_list), 0)
                        feat_H_local = torch.cat(tuple(temp_feat_H_local_list), 0)
                        ...
                        lmiloss, gmiloss, tmiloss = self.MILoss(
                            feat_L_local, feat_L_local_prime, feat_H_local) # [4, 256]

                        cnt_tmiloss += 1
                        mi_loss += tmiloss

            mi_loss = mi_loss/cnt_tmiloss
            # print(mi_loss)
        else:
            mi_loss = 0
        ...
        loss = (1-alpha)*(loss + kd_loss) + alpha * mi_loss
        ...
        if self.MGDA:
            ...
            if self.MMI_flag:
                self.optimizer_miloss.zero_grad()
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
            if self.MMI_flag:
                self.optimizer_miloss.step()
        ...
        if self.config.pred_type in ["motion", "center"] and not self.only_det:
            ...
            if self.MMI_flag:
                return loss.item(), loss_cls.item(), loss_loc.item(), mi_loss.item() #38
            else:
                return loss.item(), loss_cls.item(), loss_loc.item(), 0
